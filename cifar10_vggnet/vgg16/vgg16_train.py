...
with tf.Session() as sess:
    ...
    vgg = vgg16.Vgg16(vgg16_npy_path=extract_cifar10.vgg16_npy_path)
    ...
    for i in range(max_step):
        ...
        # 每迭代1000个 batch，对当前训练数据进行测试，输出当前预测准确率
        if i % 100 == 0:
            # Train_accuracy:
            vgg_out = sess.run(vgg.relu6, feed_dict={tf_x: batch_xs})
        ...
        # 训练的过程中每隔1000step计算test_accuracy
        # if (i + 1) % 1000 == 0:
        #     # Test_accuracy
        #     avg = 0
        #     for j in range(20):
        ...
        #     _, result = sess.run(
        #         [train_step, merge_op], {x_: vgg_out_test, tf_y: test_labels[j * batch_size:j * batch_size + batch_size], keep_prob: 1.0})
